...
import { OllamaCallbacks, OllamaSendFunction } from "../../types/ollama";
...
export const sendRequest: OllamaSendFunction = async ({ axios, callbacks, body, others, tools, functions }) => {
    try {
        ...
        while (true) {
            ...
            const { data } = await axios.post<typeof payload, AxiosResponse<Readable>>(
                "",
                payload,
                {
...
function sliceBalancedJson(content: string, startIndex: number): { json: string; endIndex: number } | null {
    let depth = 0;
    let inString = false;
